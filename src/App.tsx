...
const App: React.FC = () => {
  ...
  const [newLaptop, setNewLaptop] = useState({ brand: '', model: '', serialNumber: '', serviceId: '', specs: '' });
  ...
  const filteredLaptops = laptops.filter(laptop => {
    ...
    // 2. Prepare Data (Safe Navigation for null values)
    ...
    const specsNormalized = normalize(laptop.specs); // "16gbram,512gbssd,m1"
    
    // 3. Search Logic (Trim user input)
    ...
    // 5. Specs Logic (Using Normalization to fix "1 TB" vs "1TB")
    const matchesRam = filterRam === "ALL" || specsNormalized.includes(normalize(filterRam));
    const matchesStorage = filterStorage === "ALL" || specsNormalized.includes(normalize(filterStorage));
    const matchesCpu = filterCpu === "ALL" || specsNormalized.includes(normalize(filterCpu));
  ...
  const fetchData = async () => {
    try {
      const [laptopsRes, requestsRes, profilesRes, revisionsRes] = await Promise.all([
        ...
        supabase.from('laptop_requests').select('*').order('requested_at', { ascending: false }),
        supabase.from('profiles').select('*').order('created_at', { ascending: false }),
        supabase.from('laptop_edit_requests').select('*').order('requested_at', { ascending: false })
      ]);
...
const promoteToRoot = async (userId: string) => {
  ...
  const handleAddLaptop = async (e: React.FormEvent) => {
    ...
    setNewLaptop({ brand: '', model: '', serialNumber: '', serviceId: '', specs: '' });
    fetchData();
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 items-start">
              ...
              <div className="lg:col-span-5 bg-[#2C2C2C] rounded-[48px] p-12 text-white shadow-2xl">
                ...
                <form onSubmit={(e) => { e.preventDefault(); const found = requests.find(r => r.id === trackingId || r.employee_id === trackingId); setTrackedRequest(found || null); }} className="relative mb-8">
                  <input placeholder="Application Serial / ID" className="w-full px-8 py-5 rounded-[24px] bg-white/10 border border-white/20 text-white placeholder:text-white/30 text-sm outline-none" value={trackingId} onChange={e => setTrackingId(e.target.value)} />
                  <button className="absolute right-3 top-3 p-3 bg-white text-black rounded-2xl hover:scale-105 transition-all"><Icons.History className="w-5 h-5" /></button>
                </form>
                {trackedRequest ? (
                  <div className="bg-white/5 p-8 rounded-[32px] border border-white/10 animate-fadeIn">
                    <div className="flex justify-between items-start mb-4"><h4 className="font-black text-xl">{trackedRequest.id}</h4><StatusBadge status={trackedRequest.status} /></div>
          ...
          <div className="bg-white p-6 rounded-[32px] border border-[#EAE3D5] shadow-sm flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-3 overflow-x-auto pb-1 md:pb-0">
              ...
              <select className="px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-600 focus:outline-none cursor-pointer hover:bg-slate-100"
                ...
                <option value={LaptopStatus.MAINTENANCE}>Maintenance</option>
              </select>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl p-12 rounded-[48px] border border-[#EAE3D5] shadow-2xl relative animate-fadeInDown">
            ...
            <form onSubmit={handleAddLaptop} className="grid grid-cols-2 gap-6">
              ...
              <div className="col-span-2 flex gap-4 mt-2">
                <button type="button" onClick={() => setIsAddingLaptop(false)} className="flex-1 font-bold text-slate-400">Cancel</button>
          ...
          <div className="bg-white rounded-[48px] border border-[#EAE3D5] overflow-hidden shadow-sm">
            <table className="w-full text-left">
              <thead className="bg-[#F9F6F0] text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <tr>
                  <th className="px-12 py-10">Unit</th>
                  <th className="px-12 py-10">Service Tag</th>
                  <th className="px-12 py-10">Specifications</th>
              ...
              <tbody className="divide-y divide-[#EAE3D5]">
                {filteredLaptops.length === 0 ? (
                  ...
                  <tr key={laptop.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-12 py-8"><p className="font-black text-slate-900">{laptop.brand} {laptop.model}</p></td>
                    <td className="px-12 py-8 text-[10px] font-bold text-slate-400 uppercase tracking-widest">{laptop.service_id}</td>
                    <td className="px-12 py-8">
                       <p className="text-xs font-bold text-slate-500 truncate max-w-[200px]" title={laptop.specs}>
                         {laptop.specs || "N/A"}
                       </p>
                    </td>
                    <td className="px-12 py-8"><StatusBadge status={laptop.status} /></td>
                    <td className="px-12 py-8 text-right">
                      {assigningToRequest && laptop.status === LaptopStatus.AVAILABLE ? (
                    ...
                    <div className="flex items-center gap-12">
                      <div className="text-right">
                         ...
                         <span className={`text-[10px] font-black uppercase px-3 py-1.5 rounded-full border ${
                            rev.status === 'PENDING' ? 'bg-amber-100 text-amber-700 border-amber-200' : 
